...
const BackupLegacySettings = async (value: unknown, source: LegacySettingsBackup['source']) => {
    ...
    if (source === 'sync') {
        const local_settings = (await browser.storage.local.get(settings_key))[settings_key];
        if (isLegacySettings(local_settings) && !IsSettings(local_settings)) {
            backup_settings = local_settings;
            backup_source = 'local';
        }
    }
    const existing = (await browser.storage.local.get(legacy_backup_key))[legacy_backup_key];
    ...
    await browser.storage.local.set({ [legacy_backup_key]: backup, [migration_notice_key]: true });
};
...
export const IsSettingsSyncEnabled = async (): Promise<boolean> => {
    return (await browser.storage.local.get(sync_enabled_key))[sync_enabled_key] === true;
};
...
export const ResetSettings = async (): Promise<Settings> => {
    ...
    await browser.storage.local.set({ [sync_enabled_key]: false });
    await browser.storage.sync.remove(settings_key);
    await browser.storage.local.set({ [settings_key]: settings });
    await browser.storage.local.remove([legacy_backup_key, migration_notice_key]);
    return settings;
};
...
export const SetSettingsSyncEnabled = async (enabled: boolean): Promise<Settings> => {
    ...
    const synced = (await browser.storage.sync.get(settings_key))[settings_key];
    EnsureCurrentOperation();
...
export const RegisterSettingsSync = () => {
    browser.storage.onChanged.addListener((changes, area_name) => {
        ...
        if (changes[settings_key].newValue === undefined) {
            void browser.storage.local.set({ [sync_enabled_key]: false });
            return;
        }
        ...
        void IsSettingsSyncEnabled().then(async (enabled) => {
            ...
            await browser.storage.local.set({ [settings_key]: NormalizeSettings(synced) });
        });
    });
...
export const GetLegacySettingsBackup = async (): Promise<LegacySettingsBackup | undefined> => {
    const backup = (await browser.storage.local.get(legacy_backup_key))[legacy_backup_key];
    return isLegacySettingsBackup(backup) ? backup : undefined;
};

export const IsMigrationNoticePending = async (): Promise<boolean> => {
    const stored = await browser.storage.local.get([legacy_backup_key, migration_notice_key]);
...
export const DismissMigrationNotice = async () => {
    await browser.storage.local.set({ [migration_notice_key]: false });
};
