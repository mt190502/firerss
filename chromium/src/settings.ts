...
const ToggleSettingsSync = async (enabled: boolean) => {
    if (
        enabled &&
        !(await IsSettingsSyncEnabled()) &&
        !window.confirm(
            'Enable Browser Sync? Existing synced FireRSS settings, if present, will replace local settings.'
        )
    ) {
        ShowStatus('Browser Sync was not enabled.');
        return;
    }
