...
| **Feed Cache (Session Only)** | Positive or negative Extended Scan results (`firerss_extended:v2:<encoded-origin>`)                                                                                                                               | Background service-worker                | Stored **in-memory** in browser session storage                                                                                              |
| **Tab Feed State (Session Only)** | Passive RSS/Atom metadata URLs found for each open tab (`firerss_tab_feed_state:<tab-id>`)                                                                                                                     | Current page metadata                     | Stored in browser session storage to prevent stale results during single-page navigation; removed when the tab closes or session ends        |
| **Network Requests**          | • HTTP(S) GET requests without cookies or referrer information to: <br> – the current page after single-page navigation when live metadata is stale or missing <br> – common feed paths at website origins <br> – GitHub API and a selected theme’s GitHub-provided download URL | Initiated by content, background, or options pages | SPA page HTML is parsed transiently for RSS/Atom metadata and discarded; candidate response bodies are discarded; discovered feed URLs use the session cache, and a selected remote theme is stored as a user setting |
...
3. **Network Requests** – to retrieve additional data required for functionality:  
   • refresh RSS/Atom metadata after single-page navigation,
...
| Browser session storage (`firerss_extended:v2:*`)  | In-memory, managed by the browser          | Positive results expire after 24 hours, negative results after 15 minutes, and all entries are cleared when the browser session ends | Not written to disk                                 |
| Browser session storage (`firerss_tab_feed_state:*`) | In-memory, managed by the browser        | Retained only while its tab and browser session remain open                                                                          | Not written to disk                                 |
...
1. **Websites you load** – after a page finishes loading, a local script passively reads feed candidates already present in that page. After single-page navigation, a cookie-free and referrer-free request to the current page is used only when live feed metadata remains stale or missing. When Extended Feed Scan is enabled and the URL is not excluded, the background checks a limited set of root feed paths.
...
SPA metadata refresh and Extended Scan requests omit cookies and referrer information. SPA metadata refresh follows normal page redirects; Extended Scan rejects redirects and never reuses the current page path, query, or fragment. No additional identifiers or tokens are attached.
...
| `host_permissions` (`https://*/*`, `http://*/*`) | Enables passive candidate detection, cookie-free and referrer-free SPA metadata refresh and candidate checks, and loading optional themes from GitHub. |
