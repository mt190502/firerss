...
const injectScript = async (tab_id: number, tab_info?: browser.tabs.Tab) => {
    ...
    const injection = await browser.scripting.executeScript({
        target: { tabId: tab_id },
        func: findPassiveFeeds as unknown as () => void,
    });
...
const RunInjectScript = (tab_id: number, tab_info?: browser.tabs.Tab) => {
    void injectScript(tab_id, tab_info).catch((error: unknown) => {
...
browser.tabs.onUpdated.addListener((tab_id, status, tab_info) => {
    ...
    RunInjectScript(tab_id, tab_info);
});
